...
export class Thermal {
  ...
  public async prePrint() {
    if (this.products.length === 0) {
      throw new Error("No products to pre-print");
    }
  
    // --- Group products by name + descriptions ---
    const grouped: Record<string, { totalQty: number; descriptions: Record<string, number> }> = {};
  
    for (const p of this.products) {
      if (!grouped[p.name]) {
        grouped[p.name] = { totalQty: 0, descriptions: {} };
      }
      grouped[p.name].totalQty += p.qty;
  
      if (p.description) {
        grouped[p.name].descriptions[p.description] =
          (grouped[p.name].descriptions[p.description] || 0) + p.qty;
      }
    }
  
    // --- Reset canvas ---
    this.calculateHeight(-700);
    ...
    for (const [name, data] of Object.entries(grouped)) {
      // Main row: product name
      this.ctx.font = "bold 32px Arial";
      this.ctx.textAlign = "left";
      this.ctx.fillText(`${index}. ${name}`, this.margin, this.Y);
      this.ctx.textAlign = "right";
      this.ctx.fillText(`${data.totalQty}x`, this.width - this.margin, this.Y);
      this.Y += 40;
  
      // Subrows: descriptions (with word wrapping)
      this.ctx.font = "24px Arial";
      for (const [desc, qty] of Object.entries(data.descriptions)) {
        const lines = this.wrapText(desc, 40); // wrap description into multiple lines
  
        for (let i = 0; i < lines.length; i++) {
          this.ctx.textAlign = "left";
          this.ctx.fillText(
            i === 0 ? `- ${lines[i]}` : `  ${lines[i]}`, // indent wrapped lines
            this.margin + 40,
            this.Y
          );
          ...
          if (i === 0) {
            this.ctx.textAlign = "right";
            this.ctx.fillText(`${qty}x`, this.width - this.margin, this.Y);
          }
          this.Y += 32;
        }
      }
  ...
  private async drawReceiptBarcode(value: string = '', text: string = '') {
    if(value + text === '') {
      text = `YSDFR-${Math.floor(Date.now() / 1000)}-0001-001-${this.info.receipt.number || '0'}`;
      const secret = 'mySuperSecretKey123!';
      const cryptoUtil = new CryptoUtil(secret);
      value = cryptoUtil.encrypt(this.info.receipt.number ?? '0');
    }
